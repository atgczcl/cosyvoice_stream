...
class CosyVoice:
    ...
    def stream_sft(self, tts_text: str, spk_id: int, stream: bool = True, speed: float = 1.0, stop_generation_flag: threading.Event = None, text_frontend: bool = True):
        """
        流式处理文本转语音的功能。
    ...
    def stream_clone(self, tts_text, prompt_text, prompt_speech_16k, stream=False, speed=1.0, stop_generation_flag: threading.Event = None, zero_shot_spk_id='', text_frontend=True):
        prompt_text = self.frontend.text_normalize(prompt_text, split=False, text_frontend=text_frontend)
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend)):
            if (not isinstance(i, Generator)) and len(i) < 0.5 * len(prompt_text):
                logging.warning('synthesis text {} too short than prompt text {}, this may lead to bad performance'.format(i, prompt_text))
